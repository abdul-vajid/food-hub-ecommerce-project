...
const { COD, ONLINE } = require("../utils/constants/paymentMethod");
...
const confirmOrder = async (orderId, paymentMethod) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (paymentMethod === COD) {
        ...
        createCashfreeOrder(orderId, paymentMethod)
...
const updateConfirmOrderForOnline = (orderId, userId) => {
  return new Promise(async (resolve, reject) => {
    try {
      await getCashFreeOrderStatus(orderId, userId) //Optional Additional check for payment status
        .then(updateOrderInSuccessScenario)
        .then(updateCouponIfApplied)
        .then(emptyCart)
